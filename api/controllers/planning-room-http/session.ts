...
import { findCanonicalUserName } from '../../utils/room-data';
...
export async function handleSessionValidation(
  ...
  if (!name || !sessionToken) {
    return createJsonResponse({ error: 'Missing user name or session token' }, 400);
  }
  ...
  const tokenValid = ctx.repository.validateSessionToken(canonicalName, sessionToken);
