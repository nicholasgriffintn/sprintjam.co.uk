...
import { signState, verifyState, escapeHtml } from "../utils/security";
...
describe("Security Utils", () => {
    ...
    test("verifyState should fail with wrong secret", async () => {
        const data = { foo: "bar" };
        const signed = await signState(data, secret);
        await expect(verifyState(signed, "wrong-secret")).rejects.toThrow("Invalid state");
    });
    ...
    test("verifyState should fail with tampered data", async () => {
        ...
        await expect(verifyState(tampered, secret)).rejects.toThrow("Invalid state");
    });
