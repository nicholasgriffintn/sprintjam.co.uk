        ...
        >
          <div>
            ...
            <div className="text-xs text-slate-600 dark:text-slate-200">
              ...
              :{" "}
              {weightedScore === null ? "N/A" : `${weightedScore.toFixed(1)}%`}
            </div>
          </div>
          ...
          <div className="flex items-center justify-end gap-2">
            {(displaySettings?.summary?.showConversionCount ?? true) &&
              (calculatedVote?.appliedConversionRules?.length ?? 0) > 0 && (
                <div className="text-xs text-slate-600 dark:text-slate-200 bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded">
        ...
        {allowScoringInfoToggle &&
          ...
          (calculatedVote || hasAnyScores) && (
            <div
              ...
              className="mt-4 p-3 bg-white/85 dark:bg-slate-900/55 border border-white/50 dark:border-white/5 shadow-[0_12px_32px_rgba(15,23,42,0.12)] backdrop-blur-xl rounded-3xl"
              role="region"
              aria-live="polite"
            >
