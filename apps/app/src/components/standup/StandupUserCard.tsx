  ...
  return (
    <article
      className={cn(
        ...
        isPresentation
          ? "border-brand-200/60 bg-gradient-to-br from-white via-brand-50/70 to-amber-50/80 p-8 shadow-[0_24px_80px_rgba(14,116,144,0.18)] dark:border-brand-400/20 dark:from-slate-950 dark:via-brand-950/20 dark:to-amber-950/10"
          : "border-black/5 bg-white/80 p-5 shadow-[0_18px_45px_rgba(15,23,42,0.08)] dark:border-white/10 dark:bg-slate-950/50",
        isFocused &&
    ...
    >
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="flex min-w-0 items-center gap-4">
          <Avatar
            ...
            className={cn(
              "shrink-0 border border-black/5 bg-brand-100 text-brand-900 dark:border-white/10 dark:bg-brand-500/20 dark:text-brand-100",
      ...
      <div className="mt-6 grid gap-5">
        ...
        <div className={cn("grid gap-4", isPresentation ? "lg:grid-cols-2" : "")}>
          <section className="rounded-3xl border border-black/5 bg-white/70 p-4 dark:border-white/10 dark:bg-white/[0.03]">
        ...
        {response.hasBlocker ? (
          <section className="rounded-3xl border border-rose-200/80 bg-rose-50/90 p-4 dark:border-rose-400/20 dark:bg-rose-950/20">
              ...
              {response.linkedTickets.map((ticket) => {
                ...
                return ticket.url ? (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex max-w-full items-center gap-2 rounded-full border border-black/5 bg-black/[0.03] px-3 py-2 text-sm text-slate-700 transition hover:border-brand-300 hover:text-brand-700 dark:border-white/10 dark:bg-white/[0.04] dark:text-slate-200 dark:hover:text-brand-200"
                  >
                    {content}
                  </a>
                ) : (
                  <div
                    key={`${response.userName}-${ticket.id}`}
                    className="inline-flex max-w-full items-center gap-2 rounded-full border border-black/5 bg-black/[0.03] px-3 py-2 text-sm text-slate-700 dark:border-white/10 dark:bg-white/[0.04] dark:text-slate-200"
                  >
                    {content}
                  </div>
                );
