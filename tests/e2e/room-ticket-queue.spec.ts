...
test.describe('Ticket Queue E2E', () => {
    test('should create auto-increment ticket on first Next Ticket click', async ({
        ...
        try {
            ...
            const summary = page.getByRole('dialog', { name: 'Review before moving on' });
            ...
            await expect(
                page.getByTestId('queue-ticket-id-current')
            ).toContainText('SPRINTJAM-002');
        } finally {
            await cleanup();
        }
    });
